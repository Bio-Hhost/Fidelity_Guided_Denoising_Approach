...
def main(args):
    ...
    for epoch in range(args.rl_warmup_epochs):
        ...
        for step in range(args.steps_per_epoch):
            ...
            if args.warmup_lambda_mode == "fixed":
                assert args.lambda_fixed is not None, "--warmup_lambda_mode fixed requires --lambda_fixed"
                random_actions = tf.fill((X_seq_b.shape[0], args.action_dim), tf.constant(args.lambda_fixed, tf.float32))
            else:
            ...
            # REWARD FIX
            # y_pred_b is the PRE-update forward pass (weights theta_t). lambda_t enters only
            # theta_{t+1}, so scoring y_pred_b makes the reward independent of the action it is
            # stored with. We score the POST-update output on the same batch instead.
            r_pre_tf = None
            ...
            y_pred_post = unet_model(X_seq_b, training=False)   # no BN/dropout -> deterministic
    ...
    for epoch in range(args.total_epochs):
        ...
        for step in range(args.steps_per_epoch):
            ...
            if args.lambda_fixed is not None:
                actions_deterministic = tf.fill((X_seq_b.shape[0], args.action_dim), tf.constant(args.lambda_fixed, tf.float32))
                actions_noisy = actions_deterministic                      # frozen: no exploration noise
            else:
        ...
        if current_epoch_reward > best_val_reward:
            ...
            if args.lambda_fixed is None:   # frozen-lambda control: no agent exists, nothing to save
    ...
    # wall-clock + hardware + full hyperparameter record
    try:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lambda_fixed", type=float, default=None,
                        help="If set, lambda is FROZEN at this value and the DDPG agent is disabled "
                             "(actor/critic never update, no exploration noise). Everything else -- the "
                             "step-based loop, generator, optimiser, LR schedule, reward-based checkpointing "
                             "and early stopping -- is byte-identical to the RL run. This is the "
                             "same-pipeline static-lambda control.")
    parser.add_argument("--warmup_lambda_mode", type=str, default="random", choices=["random", "fixed"],
                        help="Warm-up lambda: 'random' = Uniform(bounds) as in the RL method; "
                             "'fixed' = held at --lambda_fixed. Use 'fixed' to match total gradient "
                             "updates while removing the random-lambda curriculum.")
